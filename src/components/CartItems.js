...
export class CartItems extends Component {
    ...
    render() {
        ...
        const totalQuantity = data.reduce((accumulator, current) => accumulator + current.quantity, 0)
        const totalPrice = data.reduce((accumulator, current) => accumulator + current.prices[this.props.price_index].amount * current.quantity, 0)
                ...
                <div>
                    <div className='cartitems'>
                        <div>
                            ...
                            {data.map((e, i) => {
                                return (
                                    <div className='d-flex  c-items-content  justify-between'>

                                        <div>
                                            <p style={{ fontSize: '30px' }}>{e.name}</p>
                                            <p style={{ fontWeight: '800', marginBottom: '10px' }}>{fetchCurrency(e.prices)}</p>
                                            ...
                                            <div style={{ marginTop: '10px' }}>
                                                <button id='btnremove' onClick={() => removeCartItem(i)}>remove</button>
                                            </div>
                                        </div>

                                        <div>
                                            <div className='d-flex justify-between' >
                                                <div className='btn-c-qty-div' style={{ height: '100%' }}>
                                                    <div className='btn-c-qty-i' style={{ paddingTop: '10px' }} onClick={() => btnIncrement(e.id)}>
                                                    ...
                                                    <div className='btn-c-qty-i' style={{ paddingLeft: '10px', paddingTop: '10px' }}>{e.quantity}</div>
                                                    <div style={{ marginTop: '20px' }} onClick={() => btnDecrement(e.id)}>
                                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                            <path d="M8 12H16" stroke="#1D1F22" stroke-linecap="round" stroke-linejoin="round" />
                                                            <rect x="0.5" y="0.5" width="23" height="23" stroke="#1D1F22" />
                                                        </svg>
                                                ...
                                                <div id='c-items-img' style={{ marginTop: '40px', height: '50%' }}>
                            ...
                            <div className='c-total' style={{ marginTop: '20px' }}>
                                <div className='d-flex'>
                                    <p>
                                        Tax 21%:
                                    </p>
                                    <p className='bold' style={{ marginLeft: '5px' }}> $40.00</p>
                                </div>
                                <div className='d-flex'>
                                    <p>
                                        Quantity:
                                    </p>
                                    <p className='bold' style={{ marginLeft: '5px' }}> {totalQuantity}</p>
                                </div><div className='d-flex'>
                                    <p>
                                        Total:
                                    </p>
                                    <p className='bold' style={{ marginLeft: '5px' }}> {this.props.currency_name}{totalPrice}</p>
                                </div>
                            </div>
...
export default connect(mapStateToProps, { addToCart })(CartItems);